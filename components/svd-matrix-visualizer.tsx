// components/SvdMatrixVisualizerUnified.tsx
"use client"
...
import { ColorSvdData, GraySvdData, SvdData } from "@/lib/utils";

// Helper functions (sliceMatrix, createSMatrixDynamic) should be here or imported

interface SvdMatrixVisualizerUnifiedProps {
    svdDataSource?: GraySvdData | ColorSvdData | null; // Can be for gray or color
    useColor: boolean;
    usedValues: number; // k
    originalRows: number;
    originalCols: number;
...
export function SvdMatrixVisualizerUnified({
    svdDataSource,
    useColor,
    usedValues,
    originalRows,
    ...
    maxCellDisplay = 15,
    matrixCellSize = 14,
}: SvdMatrixVisualizerUnifiedProps) {
    const [activeChannel, setActiveColorChannel] = useState<'R' | 'G' | 'B'>('R');

    const currentSvdToDisplay = useMemo((): SvdData | null => {
        if (!svdDataSource) return null;
        if (!useColor) {
            // Assume svdDataSource is SvdComponents for grayscale
            return svdDataSource as SvdData;
        }
        if (useColor) {
            const colorData = svdDataSource as ColorSvdData;
            if (!colorData) return null;
            const key = activeChannel.toLowerCase() as 'r' | 'g' | 'b';
            return colorData[key] || null;
        }
        return null;
    }, [svdDataSource, useColor, activeChannel]);


    if (!currentSvdToDisplay || !currentSvdToDisplay.s || !currentSvdToDisplay.u || !currentSvdToDisplay.v ||
        currentSvdToDisplay.u.length === 0 || currentSvdToDisplay.v.length === 0) {
        return (
            <p className="text-muted-foreground text-sm p-4 text-center">
                SVD data for {useColor ? `${activeChannel} channel` : 'grayscale'} incomplete or invalid.
            </p>
        );
    }
    ...
    const { u: full_u, s: full_s, v: full_vt } = currentSvdToDisplay;
                        ...
                        return (
                            <div key={channel} className="flex items-center">
                                <Button
                                    ...
                                    size="sm"
                                    onClick={() => setActiveColorChannel(channel)}
