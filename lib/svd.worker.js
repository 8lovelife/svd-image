...
self.onmessage = async (event) => {
    ...
    if (type === 'CALCULATE_SVD') {
        ...
        try {
            ...
            console.log("Worker: Starting SVD calculation for image data:", width, "x", height, "k:", k_rank_for_randomizedSVD);

            // Create reconstructed matrices for each channel
            // if (useColor) {
            console.log("Calculate color image SVD data")
            const colorSvdResult = await performColorSVD(rawImageData, k_rank_for_randomizedSVD);
            ...
            const grayscaleSvdResult = await performGrayscaleSVD(rawImageData, k_rank_for_randomizedSVD);
